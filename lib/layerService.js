...
const Logger = require('./log/Logger')('Layer-Service');
...
const init = async () => {
  ...
  if (entryCount === 0) {
    Logger.info('No entry previously saved. Will load defaults from profile');
...
const addLayer = async (layer) => {
  ...
  if (dbLayer) {
    throw {"status": 400, "message": "layer.exists"};
  }
...
const updateLayer = async (layer) => {
  ...
  let dbLayer = await Layer.Model.findOne({"meta.name": {$eq: layer.meta.name}});
  dbLayer.meta.label = l.meta.label;
  dbLayer.meta.display = l.meta.display;
  dbLayer.meta.position = l.meta.position;
  dbLayer.meta.retina = l.meta.retina;
  dbLayer.meta.type = l.meta.type;
  dbLayer.meta.source = l.meta.source;
  dbLayer.meta.vector = l.meta.vector;
  let newLayer = await dbLayer.save();
  if (l.style) {
    await new Style.Model({layerName: newLayer.meta.name,layerId: newLayer._id, style: l.style}).save();
  }
  return newLayer;
 ...
 * @returns {Promise<*>}
 */
const showLayer = async (layerName, show) => {
  await Layer.Model.update({"meta.name": {$eq: layerName}}, {$set: {"meta.display": show}});
  return await getLayer(layerName);
};
...
const computeStyle = (layer, originalStyle) => {
  ...
  style.glyphs = baseUrl + '/glyphs/' + layer.meta.name + '/{fontstack}/{range}.pbf';
  style.sprite = baseUrl + '/sprites/' + layer.meta.name;
  style.sources = {};
  for (let k in originalStyle.sources) {
    style.sources[k] = Object.assign({}, originalStyle.sources[k]);
    style.sources[k].tiles = [];
    style.sources[k].tiles = [baseUrl + '/maps/' + layer.meta.name + '-' + k + '/{z}/{x}/{y}.pbf']
  }
  return style;
};
